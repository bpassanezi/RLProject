...
def run_experiment(algo_type, route_file, experiment_type, TEST_MODE=False):
    ...
    callback = RLTrafficCallback(algo_name=algo_type, experiment_type=experiment_type)

    if algo_type == "DQN":
        # Off-policy (Stationary specialist)
        model = DQN("MlpPolicy", env, verbose=1, learning_rate=0.001, buffer_size=10000)
    else:
        # A2C (On-policy / SARSA logic - Adaptive specialist)
